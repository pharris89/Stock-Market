...
class StockApp:
    def __init__(self):
        ...
        # Create Window
        self.root = Tk()
        self.root.title("Paula's Stock Manager")
        ...
        self.webmenu.add_command(
            label="Import CSV from Yahoo! Finance...", command=self.importCSV_web_data)
        self.menubar.add_cascade(label="Web", menu=self.webmenu)

        self.chartmenu = Menu(self.menubar, tearoff=0)
        self.chartmenu.add_command(
            label="Display Stock Chart", command=self.display_chart)
        ...
        # Set Up Main Tab
        self.addStockGroup = LabelFrame(
            self.mainFrame, text="Add Stock", padx=5, pady=5)
        ...
        self.addStockButton = Button(
            self.addStockGroup, text="New Stock", command=self.add_stock)
        self.addStockButton.grid(column=0, row=3, columnspan=2, padx=5, pady=5)

        self.deleteGroup = LabelFrame(
            self.mainFrame, text="Delete Stock", padx=5, pady=5)
        self.deleteGroup.grid(column=0, row=2, padx=5, pady=5, sticky=(W, E))

        self.deleteStockButton = Button(
            self.deleteGroup, text="Delete Selected Stock", command=self.delete_stock)
        ...
        if (self.stockList.curselection()):
            ...
            for stock in self.stock_list:
                if stock.symbol == symbol:
                    self.headingLabel['text'] = stock.name + \
                        " - " + str(stock.shares) + " Shares"
                    ...
                    for daily_data in stock.DataList:
                        row = daily_data.date + "   " + \
                            '${:0,.2f}'.format(
                                daily_data.close) + "   " + str(daily_data.volume) + "\n"
                    ...
                    if count > 0:
                        ...
                        self.stockReport.insert(
                            END, "Low Price: " + "${:,.2f}".format(lowPrice) + "\n")
                        self.stockReport.insert(
                            END, "High Price: " + "${:,.2f}".format(highPrice) + "\n")
                        self.stockReport.insert(
                            END, "Average Price: " + "${:,.2f}".format(price_total/count) + "\n\n")
                        self.stockReport.insert(
                            END, "Low Volume: " + str(lowVolume) + "\n")
                        self.stockReport.insert(
                            END, "High Volume: " + str(highVolume) + "\n")
                        self.stockReport.insert(
                            END, "Average Volume: " + "${:,.2f}".format(volume_total/count) + "\n\n")
                        self.stockReport.insert(
                            END, "Change in Price: " + "${:,.2f}".format(priceChange) + "\n")
                        self.stockReport.insert(
                            END, "Profit/Loss: " + "${:,.2f}".format(priceChange * stock.shares) + "\n")
                    else:
    ...
    def add_stock(self):
        new_stock = Stock(self.addSymbolEntry.get(
        ), self.addNameEntry.get(), float(self.addSharesEntry.get()))
    ...
    def importCSV_web_data(self):
        ...
        filename = filedialog.askopenfilename(
            title="Select " + symbol + " File to Import", filetypes=[('Yahoo Finance! CSV', '*.csv')])
        if filename != "":
